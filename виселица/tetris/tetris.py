...
pygame.display.set_caption("Tetris Pirany Game")
...
for i in range(0, len(details)):
    for j in range(0, 4):
        det[i].append(
            pygame.Rect(
                details[i][j][0] * dx + dx * (I_max // 2), details[i][j][1] * dy, dx, dy
            )
        )
...
while game:
    ...
    for i in range(4):
        if (
            det_choice[i].x + delta_x * dx < 0
            or det_choice[i].x + delta_x * dx >= screen_x
        ):
            delta_x = 0
        if (det_choice[i].y + dy >= screen_y) or (
            grid[int(det_choice[i].x // dx)][int(det_choice[i].y // dy) + 1][0] == 0
        ):
            delta_y = 0
            for i in range(4):
    ...
    if rotate == True:
        ...
        rotate = False
    for j in range(J_max - 1, -1, -1):  # Циклпо рядам снизу вверх
        count_cells = 0
